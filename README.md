...
`clash select` 会通过 Mihomo 控制接口测试 `PROXY` 组内的节点，按延迟从低到高展示。超时或不可用节点不会进入选择列表；选择延迟达到 `1500 ms` 的节点时会要求再次确认。默认测试地址为 `https://www.gstatic.com/generate_204`，超时时间为 `5000 ms`。
